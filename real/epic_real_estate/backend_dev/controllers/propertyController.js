...
//get counts of types -> ex: {beach: 2, village: 10, mountain: 6}
...
propertyController.put("/:id", verifyToken, async(req, res) => {
    try {
        ...
        if(property.currentOwner !== req.user.id){
            throw new Error("Not allowed to update other people properties")
        } else{
...
propertyController.delete("/:id", verifyToken, async(req, res) => {
    try {
        ...
        if(property.currentOwner.toString() !== req.user.id.toString()){
            throw new Error("Not allowed to delete other people property")
        } else {
